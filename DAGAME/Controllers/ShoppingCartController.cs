        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    order.TotalAmount = cart.Items.Sum(x => (x.Price));
                    ...
                    foreach (var sp in cart.Items)
                    {
                        ...
                        strSanPham += "<td>" + DAGAME.Common.Common.FormatNumber(sp.TotalPrice, 0) + "</td>";
                        strSanPham += "</tr>";
                        thanhtien += sp.Price;
                    }
                    TongTien = thanhtien;
                    ...
                    contentCustomer = contentCustomer.Replace("{{ThanhTien}}", DAGAME.Common.Common.FormatNumber(thanhtien, 0));
                    contentCustomer = contentCustomer.Replace("{{TongTien}}", DAGAME.Common.Common.FormatNumber(TongTien, 0));
                    DAGAME.Common.Common.SendMail("Duckie Shop", "Đơn hàng #" + order.Code, contentCustomer.ToString(), req.Email);
                    ...
                    contentAdmin = contentAdmin.Replace("{{ThanhTien}}", DAGAME.Common.Common.FormatNumber(thanhtien, 0));
                    contentAdmin = contentAdmin.Replace("{{TongTien}}", DAGAME.Common.Common.FormatNumber(TongTien, 0));
                    DAGAME.Common.Common.SendMail("Duckie Shop", "Đơn hàng mới #" + order.Code, contentAdmin.ToString(), ConfigurationManager.AppSettings["EmailAdmin"]);
                    cart.ClearCart();
